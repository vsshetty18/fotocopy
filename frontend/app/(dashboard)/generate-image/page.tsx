...
export default function GenerateImagePage() {
  ...
  async function fetchHistory() {
    try {
      const response = await api.get<ApiResponse<{ images: GeneratedImage[] }>>("/generate");
  ...
  async function handleGenerate() {
    ...
    try {
      const response = await api.post<ApiResponse<GeneratedImage>>("/generate", { prompt });
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-muted mt-1">Describe an image and let AI create it for you</p>
      </div>
      ...
      <div className="mt-8">
        <h2 className="text-lg font-semibold text-foreground mb-4">Your generated images</h2>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {history.map((image) => (
              <Card key={image.id} padded={false} className="overflow-hidden">
                ...
                <div className="p-3">
                  <p className="text-xs text-muted line-clamp-2 mb-2">{image.prompt}</p>
